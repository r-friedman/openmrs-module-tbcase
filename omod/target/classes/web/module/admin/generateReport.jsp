...
<spring:hasBindErrors name="TbcaseReport">
...
<h3><c:out value="${TBCaseReport.name}" /></h3>
<p><c:out value="${TBCaseReport.description}"/></p>


<form method="post">
<fieldset>
<c:forEach var="p" items="${TBCaseReport.parameters}">
<c:if test='${p!=null}'><c:set var="nonNullValueExists" value="true"></c:set></c:if>
</c:forEach>
<c:if test='${not empty TBCaseReport.parameters and nonNullValueExists}'>
Enter the following parameters:
<table>
	<c:forEach var="parameter" items="${TBCaseReport.parameters}" varStatus="varStatus">
	<c:if test='${parameter!=null}'>
		<c:if test="${parameter.visible}">
			<tr>
				<td><c:out value="${parameter.displayName}" /></td>
				<spring:nestedPath path="TBCaseReport.parameters[${varStatus.index}]">
					<td><c:choose>
						<c:when
